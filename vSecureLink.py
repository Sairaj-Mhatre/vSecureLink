...
root = tk.Tk()
root.title("vSecureLink")
root.geometry('400x500')

root.configure(bg='#1a75ff')

def configure_widget(widget, font=('Helvetica', 10, 'bold'), bg='#1a75ff', fg='white'):
    widget.configure(bg=bg, fg=fg, font=font)
    if isinstance(widget, tk.Entry):
        widget.configure(insertbackground='white')

mode_label = tk.Label(root, text="vSecureLink by Vidyalankar", font=('Helvetica', 16, 'bold'))
configure_widget(mode_label)
mode_label.pack(pady=10)

website_label = tk.Label(root, text="www.vit.edu.in", font=('Helvetica', 10, 'italic'))
